...
def frame_detections(path, sess, frame_count, width, height, det):
  ...
  for i in range(len(scores)):
    if scores[i] >= threshold and dclses[i] in ids:
      ...
      bb_left, bb_top, bb_width, bb_height = height * bboxes[i][0], width * bboxes[i][1], height * abs(bboxes[i][0] - bboxes[i][2]), width * abs(bboxes[i][1] - bboxes[i][3])
      ...
      print((width, height))
      print('{}, {}, {}, {}, {}, {}, {}, {}\n'.format(bb_left, bb_top, bb_width, bb_height, scores[i], -1, -1, -1))
      # close file
...
with detection_graph.as_default():
  with tf.Session() as sess:
    ...
    frame_detections(None, sess, frame_count, width, height, None)
